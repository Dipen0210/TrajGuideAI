...
2. Tools call a hosted Llama 3 endpoint for natural-language generation.
3. Retrieved RAG sources and explanations supplement the core trajectory outputs.

## Backend + UI
1. **FastAPI** hosts `/predict`, `/explain`, `/risk`, handling JSON schemas defined in `api/schemas.py`.
2. **Streamlit** consumes the API, enabling CSV uploads, manual JSON input, charts, risk gauges, and narrative explanations.
